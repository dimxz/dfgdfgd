...
//------------------------------------------------------
let phoneNumber = "+62858135194762"
let owner = JSON.parse(fs.readFileSync('./src/data/role/owner.json'))

const pairingCode = !!phoneNumber || process.argv.includes("")
...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`Nih pairing code mu  : `)), chalk.black(chalk.white(code)))
      }, 3000)
   }
...
try{
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.badSession) {
				console.log(`File session rusak / tidak valid, hapus terlebih dahulu.`);
				startXeonBotInc()
				...
				console.log("Koneksi ditutup, sesi baru dibuka, tutup terlebih dahulu sesi saat ini.");
				startXeonBotInc()
			} else if (reason === DisconnectReason.loggedOut) {
				console.log(`Perangkat telah di logout, hapus file session terlebih dahulu.`);
				startXeonBotInc();
			} else if (reason === DisconnectReason.restartRequired) {
				console.log("Restart dibutuhkan, memulai ulang...");
				startXeonBotInc();
			} else if (reason === DisconnectReason.timedOut) {
				console.log("Waktu koneksi habis, menghubungkan ulang...");
				startXeonBotInc();
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`Terhubung ke => ` + JSON.stringify(XeonBotInc.user, null, 2), 'yellow'))
			await delay(1999)
...
const xeondate = moment.tz('Asia/Jakarta').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `𝗢𝗼𝗽𝘀‼️ @${xeonName.split("@")[0]}, mampua bukan admin lagi 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				...
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nGrup.telah ditutup oleh admin, sekarang hanya admin yang dapat mengirimkan pesan !`,
				})
			} else if (res.announce == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nGrup sudah dibuka oeh admin, semua anggota grup bisa mengirimkan pesan !`,
				})
			} else if (res.restrict == true) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nInfo grup sudah dibatasi, hanya owner yang dapat mengedit info grup !`,
				})
			} else if (res.restrict == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nPembatasam info grup sudah diubah, semua orang bisa mengedit info grup!`,
				})
			} else if(!res.desc == ''){
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, { 
					text: `「 Group Settings Change 」\n\n*Deskripsi grup sudah diubah menjadi : *\n\n${res.desc}`,
				})
      } else {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\n*Nama grup diubah menjadi : *\n\n*${res.subject}*`,
				})
			} 
			}
		})
